...
- **Toss (strip):** Show/Hide puts the full toss result in the **right bowler panel** (replaces name/figures/this-over). Center notch keeps normal CRR/NEED. Clears on take-off, Hide, or the next ball.
- **Tournament (full-screen):** Point Table, Top 5 Batsmen/Bowlers, Tournament Fours/Sixes — fetched from `GET /tournaments/:id/standings`, `/leaderboard`, `/stats` (tournament aggregates, not match scorecard).
