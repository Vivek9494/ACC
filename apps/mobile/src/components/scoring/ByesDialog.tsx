...
/** Bye run options shown in the 3×2 grid (1 B … 6 B). */
export const BYE_VALUES: readonly (readonly number[])[] = [
  [1, 2, 3],
  [4, 5, 6],
] as const;

export interface ByesDialogProps {
  visible: boolean;
  onCancel: () => void;
  onSelect: (runs: number) => void;
}

function formatByeLabel(value: number): string {
  return `${value} B`;
}

/** Bye run picker (§12.1) — extras to the team, legal ball, not off the bat. */
export function ByesDialog({
  visible,
  onCancel,
  onSelect,
}: ByesDialogProps): React.ReactElement {
  return (
      ...
      <Pressable className="flex-1 items-center justify-center bg-black/40 px-4" onPress={onCancel}>
        ...
        >
          <View className="flex-row items-center justify-between border-b border-outline-variant px-4 py-3">
            <Text className="font-sans-bold text-lg text-on-surface">BYES</Text>
            <Pressable
              onPress={onCancel}
              ...
              <View key={row.join(',')} className="flex-row gap-2">
                {row.map((value) => (
                  <Pressable
                    ...
                    accessibilityRole="button"
                    accessibilityLabel={`Byes ${formatByeLabel(value)}`}
                  >
                    <Text className="font-sans-bold text-xl text-on-surface">
                      {formatByeLabel(value)}
                    </Text>
                  </Pressable>
                ))}
              </View>
